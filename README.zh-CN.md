...
- **执行扩展命令**：pi 扩展注册的斜杠命令（如 `/reasonix-status`、`/perm`）可以直接在输入框执行，多行命令输出会在通知栏完整展示，不再被截断。
- **审批前可查看**：权限 allow/deny 弹窗以非模态卡片停靠在输入框上方——聊天历史保持可见可滚动，原始请求（bash 命令/文件路径）默认一行截断显示，可展开查看完整内容。
